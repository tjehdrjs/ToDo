...
# 이 파일은 데이터 베이스 테이블을 초기화(삭제 후 재생성)하는 스크립트이다.
# - 수업이나 개발 과정에서 테이블 구조를 바꾼 뒤 다시 적용할 떄 사용한다.
# - 기존 테이블을 모구 삭제(drop)한 후 새로 생성(create)한다.
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# PostgresSQL 연결 주소 설정 (동기용 드라이버 사용)
...
# * echo=True - 실행되는 SQL문이 터미널에 풀력됨 (디버깅에 유용)
# ---------------------------------------------------------------
