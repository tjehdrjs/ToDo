...
# ------------------------------------------------------------------
# 이 테스트 함수는 사용자가 할 일을 등록할 떄 입력하는 `due_date` 값이
# 유효하지 않는 날짜이거나 올바른 형식이 아닐 경우 서버가 어떻게 반응하는지 확인합니다.
#
# [검사 항목]
# (1) 올바른 날짜 입력 시 정상 처리 (200 OK)
# (2) 존재하지 않는 날짜 (예: 12월 32일) - 오류반환 (422)
# (3) 잘못된 구분자 사용 (예: 2004/12/01) - 오류반환 (422)
# (4) 구분자 없이 숫자만 나열 (예: 20241201) - 오류반환 (422)
#
