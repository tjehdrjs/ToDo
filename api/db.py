...
# 파일명: db.py
# 위치: api/db.py
# 이 파일은 FastAPI 앱에서 사용할 데이터 베이스와 연결하고,
...
# - 우리가 만들 테이블들은 이 클래스를 '기반으로' 정의하세 된다.
from sqlalchemy.orm import sessionmaker, declarative_base

# -------------------------------------------------------------
# [1] PostgresSQL에 연결할 주소 설정 (DB 접속 정보)
# 형식: Postgresql+asyncpg:// 사용자: 비밀번호@호스트/데이터베이스이름
# - postgresql+asyncpg : 비동기 방식으로 PostgresSQL에 접속할 수 있게 해주는 형식
# - todo_user : DB 사용자 이름
# - 1234 : 사용자 비밀번호
...
# -------------------------------------------------------------
# [2] DB 엔진 생성
# - 엔진은 FastAPL와 PostgresSQL 사이를 연결해주는 역활을 한다.
...
# - autocommit=False : 자동 저장하지 않음 (commit()을 직접 호줄해야 저장됨)
...
# - 이후에 만들 모든 테이블 클래스들은 이 Base응 상속받아 정의한다.
